...
public class ArticuloService {
    ...
    public ArticuloDTO consultarArticuloDTO(Integer idArticulo) {
        String url = "http://localhost:8085/api/articulos/" + idArticulo;
        Mono<ArticuloDTO> response = this.webClientBuilder.build().get().uri(url).retrieve().bodyToMono(ArticuloDTO.class);
        return response.block();
    }

    public ArticuloDTO actualizarArticuloDTO(ArticuloDTO articulo, Integer id) {
        String url = "http://localhost:8085/api/articulos/" + id;
        Mono<ArticuloDTO> response = this.webClientBuilder.build().put().uri(url).body(Mono.just(articulo), ArticuloDTO.class).retrieve().bodyToMono(ArticuloDTO.class);
        return response.block();
    }

    public List<ArticuloDTO> listarArticulosDeRevisor(Integer idRevisor) {
        String url = "http://localhost:8085/api/articulos/revisor/" + idRevisor;
        Mono<ArticuloDTO[]> response = this.webClientBuilder.build().get().uri(url).retrieve().bodyToMono(ArticuloDTO[].class);
